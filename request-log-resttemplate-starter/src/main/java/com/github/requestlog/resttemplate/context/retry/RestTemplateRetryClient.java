 ...
 */
public class RestTemplateRetryClient extends RetryClient {
    ...
    public RestTemplateRetryClient client(RestTemplate restTemplate) {
    ...
    public RetryResult execute() {
        ...
        try {
            ClientHttpResponse response = restTemplate.execute(retryContext.buildRequestUrl(), HttpMethod.valueOf(retryContext.getRequestLog().getHttpMethod().name()), (clientHttpRequest) -> {
                requestHolder.set(clientHttpRequest);
                restTemplate.httpEntityCallback(requestEntity).doWithRequest(clientHttpRequest);
